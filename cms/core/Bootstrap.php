...
class Bootstrap{
	...
	public static function run(){
		...
		$data = new classes\Data();
		
    	//load request get y post
		$data->setRequest(self::loadRequest());
		
		//load data bbdd and instance controller
		$data = self::loadData($data);
		\core\util\Log::add($data, false);
		
		$nombreController = "\\".$data->getRequest()->getApplication()."\\controllers\\".$data->getModulo()."Controller";
		$controller = new $nombreController($data);
    ...
    */
     ...
     * @return Data
     */
    private static function loadData(classes\Data $data){
    ...
    private static function loadDataWWW(classes\Data $data){
    	...
    	try{
			...
			util\Error::add("<strong>error ".__FUNCTION__. "</strong><br>". $e->getMessage());
		}
		return $data;
    }
