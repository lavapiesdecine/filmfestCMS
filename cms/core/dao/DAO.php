...
class DAO extends Database{
	...
	public function paginaByIdDAO($controller, $id, $anyo){
		if ($controller=='pelicula'){
			$sql = "select 0 id, muestra as anyo, 'pelicula' modulo, 'pelicula.phtml' layout, muestra skin, 0 id_texto, 0 id_paginapadre from peliculas where id='$id'";
		} else if($controller=='proyecciones'){
			$sql = "select 0 id, anyo, 'proyecciones' modulo, '2columnas.phtml' layout, anyo skin, 0 id_texto, 0 id_paginapadre from proyecciones ".($id>0 ? "where DATE_FORMAT(dia, '%d%m%Y') ='$id'" : "where anyo ='$anyo'");
		}
		\core\util\Log::add($sql, false);
	...
	public function edicionesDAO(){
		return parent::selectQuery("select id, nombre, descripcion, cartel, DATE_FORMAT(fecha_inicio, '%d/%c') as inicio, DATE_FORMAT(fecha_fin, '%d/%c') as fin from ediciones e where e.alta='S' order by e.fecha_inicio asc", true, __FUNCTION__);
	}
	...
	public function menuDAO($anyo){
		return parent::selectQuery("select p.id, p.url, p.id_paginapadre as modulo_padre, m.portada from pagina p, menu m where p.id=m.id_pagina and p.alta='S' and p.muestra='$anyo' and id_paginapadre=0 order by m.orden", true, __FUNCTION__.$anyo);
	}
	public function submenuDAO($idPaginaPadre){
		return parent::selectQuery("select p.id, p.url, m.orden as orden, p.id_paginapadre as modulo_padre, m.portada from pagina p, menu m where p.id=m.id_pagina and p.alta='S' and p.id_paginapadre=$idPaginaPadre order by m.orden", true, __FUNCTION__.$idPaginaPadre);
	}
	
	/*mail confirmacion */
	public function datosContactoDAO($idPelicula){
		return parent::selectQuery("select p.titulo, a.email, a.autor from autores a, peliculas p where p.id=a.id_pelicula and a.id_pelicula=$idPelicula", false, __FUNCTION__.$idPelicula); 	   		
	}
    
	/* material inscrito */
	public function numeroAutoproduccionesDAO($anyo){
		return parent::selectQuery("select p.titulo from peliculas p, convocatoria c where p.id=c.id_pelicula and p.muestra='$anyo' and c.alta='S'", true, __FUNCTION__.$anyo);
	}
	...
	public function espaciosDAO($anyo){
		return parent::selectQuery("select * from espacios where alta='S' and id in (select id_espacio from proyecciones where anyo='$anyo')", true, __FUNCTION__.$anyo);
	}
	...
	public function proyeccionesDAO($diaProyeccion, $anyo){
		$sql = "select pr.titulo, pr.dia, TIME_FORMAT(pr.hora,'%H:%i') as hora, e.espacio, e.direccion, e.id as id_espacio, pr.id as id_proyeccion from espacios e, proyecciones pr where e.id=pr.id_espacio and pr.alta='S'";
